defmodule AgentCom.ClaudeClient.Prompt do
  ...
  def build(:generate_proposals, %{context: context}) do
    ...
    """
    You are a contemplation agent. Your task is to generate feature proposals
    for the AgentCom autonomous hub based on patterns observed during operation.

    <tech-stack>#{escape_xml(to_string(tech_stack))}</tech-stack>

    <codebase-summary>
    #{escape_xml(to_string(codebase_summary))}
    </codebase-summary>

    <fsm-history>
    #{escape_xml(to_string(fsm_history))}
    </fsm-history>

    <out-of-scope>
    #{escape_xml(to_string(out_of_scope))}
    </out-of-scope>
    ...
    Generate up to 3 feature proposals that would improve the system. Each proposal
    should be concrete, actionable, and based on observable patterns. Focus on:
    - Reliability improvements (error handling, resilience)
    - Performance optimizations (throughput, latency)
    - Observability gaps (metrics, logging, alerting)
    - Developer experience (tooling, documentation)

    Each proposal must be achievable in one milestone phase. Do NOT propose features
    that are listed in the out-of-scope section. Ground proposals in actual codebase files.

    ## Response Format

    Respond with a `<proposals>` root element containing `<proposal>` children.
    ...
    <proposals>
      <proposal>
        ...
        <problem>CLI invocations fail silently under sustained load</problem>
        <solution>Implement circuit breaker pattern with configurable thresholds</solution>
        <description>Implement circuit breaker pattern for Claude Code CLI invocations.</description>
        <rationale>Observed repeated CLI timeouts during high load periods.</rationale>
        <why-now>Load has increased since adding goal orchestration</why-now>
        <why-not>Adds complexity to the CLI call path</why-not>
        <impact>high</impact>
