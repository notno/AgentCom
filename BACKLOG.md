...
- **Physical agent bodies + AR compositing** — Retro-futuristic robot bodies (TV for a head) in a physical studio. Green screen material on the TV-head face area. ArUco markers on studio walls/ceiling for camera position tracking. Camera tracking data sent over OSC to Unreal Engine. Real-time face compositing onto the green screen TV-heads using the tracking data — faces stay locked to bodies as camera moves. Full pipeline: physical robot bodies → ArUco camera tracking → OSC → Unreal → composited faces + shader-driven emotions. The agents literally have physical bodies in a studio.
