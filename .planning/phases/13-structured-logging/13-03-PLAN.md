...
must_haves:
  truths:
    ...
    - "Sidecar log output is NDJSON on stdout with consistent field conventions matching the Elixir hub, including function and line fields"
  ...
  artifacts:
    ...
    - path: "sidecar/lib/log.js"
      provides: "Structured JSON logger for Node.js sidecar with function/line fields"
  ...
  key_links:
    - from: "lib/agent_com/socket.ex"
      to: "Logger.metadata"
      via: "request_id set per incoming WS message in WebSock handle_in/2"
...
<objective>
Convert remaining Elixir modules to structured logging, add request_id generation in Socket, add runtime log-level API endpoint, and convert the Node.js sidecar to structured JSON output with full trace metadata (including function name and line number).
...
<task type="auto">
  ...
  <action>
    **All 9 modules -- common pattern:**
    1. Add `Logger.metadata(module: __MODULE__)` in init/1 (or init/1 for Socket -- see below).
    ...
    **Socket (1 Logger call -- special handling for request_id):**
    ...
    NOTE: Socket.ex uses `@behaviour WebSock` (NOT Phoenix Channels). The callbacks
    are WebSock callbacks: `init/1`, `handle_in/2`, `handle_info/2`, `terminate/2`.
    This is confirmed by the source code at lib/agent_com/socket.ex. Do NOT confuse
    with Phoenix Channel's handle_in/3.

    1. In init/1 (WebSock init -- takes one arg `_opts`), set `Logger.metadata(module: __MODULE__)`.
    2. In handle_in/2 (WebSock callback -- takes `{text, [opcode: :text]}` and `state`),
       generate and set a request_id at the TOP of the function, before JSON decoding:
       ```elixir
    ...
    3. When the agent identifies (in the `do_identify/3` helper, after successful auth),
       also set agent_id in metadata:
       ```elixir
       Logger.metadata(agent_id: agent_id)
       ```
    4. Convert the existing `log_task_event/4` helper function to use structured logging.
       Currently it does: `Logger.info("Task event: #{event} from #{agent_id} for task #{task_id}")`
       Replace with: `Logger.info(event, task_id: task_id, agent_id: agent_id, payload_type: Map.get(msg, "type"))`
  ...
  <done>
    All 9 remaining Elixir modules converted to structured logging with Logger.metadata in init.
    Socket generates request_id per WebSocket message (in WebSock handle_in/2) and sets agent_id after identification (in do_identify/3).
...
<task type="auto">
  <name>Task 2: Convert Node.js sidecar to structured JSON logging with function/line metadata</name>
  <files>
    sidecar/lib/log.js
    sidecar/index.js
  </files>
  <action>
    1. Create sidecar/lib/log.js -- a structured JSON logger matching Elixir hub field conventions.
       Per locked decision: "Full trace metadata in every log entry: timestamp, level, module,
       message, pid, node, function name, line number". The sidecar log output MUST include
       function name and line number fields to match the Elixir hub's mfa/line metadata.
  ...
  <verify>
    ...
    Manually verify: `node -e "const {initLogger, log} = require('./sidecar/lib/log'); initLogger({agent_id: 'test'}); log('info', 'test_event', {key: 'value'});"` -- should output valid JSON to stdout with all required fields (time, severity, message, module, pid, node, agent_id, function, line).
    Verify function/line fields: Parse the JSON output and confirm "function" is a string (or null for top-level) and "line" is an integer. Per locked decision, these fields MUST be present.
  ...
  <done>
    sidecar/lib/log.js created with NDJSON output, level filtering, file rotation, secret redaction, and function/line metadata capture via Error().stack.
    All console.log/console.error calls in sidecar/index.js replaced with structured log() calls.
    Sidecar log field conventions match Elixir hub (time, severity, message, module, pid, node, agent_id, function, line).
...
- Sidecar JSON output matches Elixir hub field conventions including function and line fields
</verification>

<success_criteria>
1. All 22 remaining Logger calls across 9 Elixir modules converted to structured format
2. Socket generates request_id per WS message (in WebSock handle_in/2) and sets agent_id after auth
3. PUT /api/admin/log-level endpoint operational with auth and validation
4. sidecar/lib/log.js provides structured JSON logging with same field conventions as hub, including function name and line number
