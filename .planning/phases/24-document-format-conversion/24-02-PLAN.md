...
<task type="tdd">
  <name>Test XML encode/decode round-trip for all 5 schema types</name>
  <files>
    test/agent_com/xml/xml_test.exs
    test/agent_com/xml/parser_test.exs
    test/agent_com/xml/schemas/goal_test.exs
  ...
  <verify>
    Run `mix test test/agent_com/xml/` passes with all tests green.
    At least 15 test cases across the 5 test files.
    Round-trip tests exist for all 5 schema types.
    Error handling tests exist for malformed input.
    List field tests exist for Goal (success_criteria), Proposal (related_files), FsmSnapshot (transition_history).
  </verify>
  <done>
    All XML tests pass. Round-trip encode/decode proven correct for all 5 schema types. List fields survive serialization. Error tuples returned for invalid input. Required field validation tested.
  </done>
</task>
