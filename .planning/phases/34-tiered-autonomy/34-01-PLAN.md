...
<task type="tdd">
  ...
  <action>
    Execute TDD RED-GREEN-REFACTOR cycle using the behavior and implementation sections above.
    RED: Write failing tests for all cases in behavior section (tier 1/2/3 classification, edge cases, config overrides).
    GREEN: Implement RiskClassifier.classify/2 following implementation section (gather-signals-then-classify pattern).
  ...
  <done>
    RiskClassifier.classify/2 correctly classifies into tiers 1/2/3, all thresholds config-driven, at least 15 test cases passing, 2-3 TDD commits present
  </done>
</task>
